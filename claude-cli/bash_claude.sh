...
image_ref="${CLAUDE_IMAGE_REF:-oras://ghcr.io/robot144/claude-cli:latest}"
bind_path="$(pwd -P)"
...
usage() {
  cat <<'EOF'
Usage: bash_claude.sh [--bind PATH|HOST:CONTAINER] [--bind-ro PATH|HOST:CONTAINER] \
...
# Claude-specific: Claude Code documents CLAUDE.md rather than AGENTS.md.
exec apptainer exec --no-home \
  ...
  "${dbus_args[@]}" \
  "$image" \
  /bin/bash -lc "if [[ ! -f \"$bootstrap_sentinel_host\" ]]; then /opt/bootstrap-workspace.sh; fi; if [[ ! -f \"AGENTS.md\" && -f \"/opt/AGENTS.md\" ]]; then cp /opt/AGENTS.md AGENTS.md; fi; if [[ ! -e \"CLAUDE.md\" && -f \"AGENTS.md\" ]]; then ln -s AGENTS.md CLAUDE.md; fi; exec /bin/bash -i"
