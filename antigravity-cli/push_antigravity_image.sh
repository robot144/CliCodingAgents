...
export DEFAULT_IMAGE_PATH="${DEFAULT_IMAGE_PATH:-$script_dir/antigravity-cli.sif}"
export DEFAULT_IMAGE_NAME="${DEFAULT_IMAGE_NAME:-antigravity-cli}"
export DEFAULT_BUILD_HINT="${DEFAULT_BUILD_HINT:-./build_antigravity.sh}"

if [[ $# -eq 0 && -t 0 && -t 1 ]]; then
  default_namespace="${GHCR_NAMESPACE:-robot144}"
  default_image_name="${GHCR_IMAGE_NAME:-antigravity-cli}"
