...
image="$repo_root/gemini-cli.sif"
bind_path="$PWD"
...
if [[ ! -f "$image" ]]; then
  ...
  echo "Build it first with: ./build_gemini.sh" >&2
  exit 1
fi
...
echo "Type 'gemini' to start Gemini CLI inside the container."
echo "Authenticate with Google on first use, or set GEMINI_API_KEY before launching."
