...
export DEFAULT_IMAGE_PATH="${DEFAULT_IMAGE_PATH:-$script_dir/mistral-cli.sif}"
export DEFAULT_IMAGE_NAME="${DEFAULT_IMAGE_NAME:-mistral-cli}"
export DEFAULT_BUILD_HINT="${DEFAULT_BUILD_HINT:-./build_mistral.sh}"

if [[ $# -eq 0 && -t 0 && -t 1 ]]; then
  default_namespace="${GHCR_NAMESPACE:-robot144}"
  default_image_name="${GHCR_IMAGE_NAME:-mistral-cli}"
